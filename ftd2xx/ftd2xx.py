...
def listDevices(flags=0):
    ...
    if devcount:
        ...
        bd = [c.c_buffer(defines.MAX_DESCRIPTION_SIZE) for i in range(devcount)] + [
            None
        ]
...
def getDeviceInfoDetail(devnum=0, update=True):
    ...
    name = c.c_buffer(defines.MAX_DESCRIPTION_SIZE)
...
if sys.platform == "win32":
    ...
    def w32CreateFile(
        name, access=GENERIC_READ | GENERIC_WRITE, flags=defines.OPEN_BY_SERIAL_NUMBER
    ):
        return FTD2XX(
